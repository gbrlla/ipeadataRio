...
pacotes<-c("dygraphs","forecast","xlsx","RODBC")
...
encontra.serie <- function(serie, plotar = TRUE)
{
  ...
  if(plotar & nrow(serinput) <= 5 & length(unique(serinput$PERID))==1)
  {
    ...
    if(ncol(aux)==2)
    {
      print(dygraphs::dySeries(dygraph = dyRangeSelector(dygraphs::dygraph(data = ts.aux)),name = "V1",label = names(aux)[-1]))
    } else {print(dygraphs::dySeries(dygraph = dyRangeSelector(dygraphs::dygraph(data = ts.aux))))}
  }
