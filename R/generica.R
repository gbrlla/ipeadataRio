...
#'
#' @param serie Vetor contendo o \code{SERCODIGOTROLL}
#' da(s) serie(s) requisitada(s).
#'
...
#' gen_verif <- generica(serie = c("GM12_DOW12", "ABATE12_ABPENO12",
#' "MTE12_SALMIN12"))
#'
#' @export

generica <- function(serie) {
  ...
  for (i in 1:length(nomes)) {
    GENERICAv <- merge(GENERICAv,
                       subset(x = serie,
                              subset = serie$SERCODIGO == nomes[i],
  ...
  if (erros > 0) {
    GENERICAv <- GENERICAv[-which(rowSums(GENERICAind) == ncol(GENERICAind)), ]
  }
